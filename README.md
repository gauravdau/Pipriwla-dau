...
<head>
  ...
  <title>Meesho Clone — Shopping Landing</title>
  <style>
    :root{--primary:#ff3b30;--muted:#6b7280;--card:#ffffff;--bg:#f7f7fb}
    *{box-sizing:border-box;font-family:Inter, system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial}
    body{margin:0;background:var(--bg);color:#111}
    header{background:linear-gradient(90deg, #fff 0%, #fff 100%);box-shadow:0 1px 8px rgba(16,24,40,0.06);position:sticky;top:0;z-index:40}
    .container{max-width:1100px;margin:0 auto;padding:0 16px}
    .topbar{display:flex;align-items:center;justify-content:space-between;padding:12px 0}
    .brand{display:flex;gap:12px;align-items:center}
    .logo{width:42px;height:42px;border-radius:8px;background:var(--primary);display:flex;align-items:center;justify-content:center;color:white;font-weight:700}
    nav{display:flex;gap:12px;align-items:center}
    .search{flex:1;display:flex;gap:8px;align-items:center;margin-left:18px}
    .search input{flex:1;padding:10px 12px;border-radius:10px;border:1px solid #e6e6ee;background:white}
    .btn{background:var(--primary);color:white;padding:10px 14px;border-radius:8px;border:none;cursor:pointer}/* hero */
.hero{display:grid;grid-template-columns:1fr 420px;gap:20px;padding:28px 0;align-items:center}
.hero-card{background:linear-gradient(180deg,#fff,#fff);padding:28px;border-radius:12px;box-shadow:0 6px 18px rgba(16,24,40,0.06)}
.hero h1{margin:0 0 10px;font-size:28px}
.hero p{margin:0;color:var(--muted)}

/* categories */
.cats{display:flex;gap:12px;margin:18px 0;overflow:auto;padding-bottom:6px}
.cat{background:white;padding:10px 12px;border-radius:10px;min-width:120px;text-align:center;box-shadow:0 4px 12px rgba(8,10,20,0.04)}

/* product grid */
.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px}
.card{background:var(--card);border-radius:12px;padding:10px;box-shadow:0 6px 18px rgba(16,24,40,0.04);display:flex;flex-direction:column}
...
.badge{background:#eefdf6;color:#037a4b;padding:6px 8px;border-radius:8px;font-weight:600}

/* cart */
.cart-panel{position:fixed;right:18px;bottom:18px;background:linear-gradient(180deg,#fff,#fff);width:340px;max-height:70vh;border-radius:12px;padding:12px;box-shadow:0 10px 30px rgba(2,6,23,0.12);overflow:auto}
...
<footer>
  ...
  </main>  <!-- cart panel (hidden by default) -->  <div id="cartPanel" class="cart-panel" style="display:none">
